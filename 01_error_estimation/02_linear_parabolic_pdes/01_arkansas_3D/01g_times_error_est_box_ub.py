...
from dolfinx.fem import functionspace, dirichletbc, create_interpolation_data
...
msh_ark = mesh.create_mesh(MPI.COMM_WORLD, cells, vertices, domain)

# initialize the bdry facets
tdim = msh_ark.topology.dim
fdim = tdim - 1
msh_ark.topology.create_connectivity(fdim, tdim)
boundary_facets = mesh.exterior_facet_indices(msh_ark.topology)


V_ark         = functionspace(msh_ark, ("CG", 1))
char_func_ark = fem.Function(V_ark)
char_func_ark.x.array.real[:] = 1.
...
# interpolate the char func
char_func     = fem.Function(V)
cells_box     = mesh.locate_entities(msh, msh.topology.dim, lambda x: x[0] > -1000)
interpol_data = create_interpolation_data(
                    char_func.function_space,
                    char_func_ark.function_space,
                    cells_box,
                    padding=1e-14)
char_func.interpolate_nonmatching(char_func_ark, interpolation_data=interpol_data, cells=cells_box)
...
def boundary_D(x):
    # x direction
    bdry_idx = np.isclose(x[0], 0)
    bdry_idx = np.logical_or(bdry_idx,np.isclose(x[0], 1.2))
    # y direction
...
for mu_idx, mu in enumerate(mu_set[0:1]):
    ...
    residual_NN = (-(nn_func.dx(2)*v*char_func \
                    + ufl.dot(ufl.dot(ufl.grad(nn_func), A_diff), ufl.grad(v))*char_func \
                    + ufl.dot(b_conv, ufl.grad(nn_func))*v*char_func \
                    + c_reac*nn_func*v*char_func) \
                   + f_source*v*char_func)*ufl.dx
    ...
    # solve
    solve_times = timeit.timeit("prob_riesz_L2_H10.solve()", 
                                setup="from __main__ import prob_riesz_L2_H10",
                                number=10)
